...
import (
	...
	"github.com/andreanpradanaa/simple-bank-app/gapi"
...
func runGRPCServer(
	...
	waitGroup.Go(func() error {
		...
		if err != nil {
			...
			log.Err(err).Msg("grpc server is failed to serve")
			return err
		}
		return nil
	})
...
func runGatewayServer(
	...
	jsonPb := runtime.WithMarshalerOption(runtime.MIMEWildcard, &runtime.JSONPb{
	...
	grpcMuxServer := runtime.NewServeMux(jsonPb)

	err = pb.RegisterSimpleBankHandlerServer(ctx, grpcMuxServer, server)
	if err != nil {
		log.Fatal().Err(err).Msg("cannot register handler server")
	}

	mux := http.NewServeMux()
	mux.Handle("/", grpcMuxServer)
...
func runDBMigration(migrationURL, DBSource string) {
	...
	if err != nil {
		log.Fatal().Err(err).Msg("error create new migration instance")
	}
